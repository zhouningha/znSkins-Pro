...
function applySkin(el, current, skin) {
    ...
    const assets = { ...(next.resource_pack.assets || {}) };
    for (const key of ['theme_css', 'stage', 'base']) {
        ...
        if (key === 'theme_css' && val.startsWith('theme.css'))
    ...
    host.root.querySelectorAll('input[data-text-path], select[data-text-path]').forEach((el) => {
        el.addEventListener('change', () => {
            ...
            if (path === 'resource_pack.skin') {
                ...
                if (skinSelectTimer)
                    window.clearTimeout(skinSelectTimer);
...
function firstToken$1(computed, keys) {
...
function syncPortalThemeVariables(host) {
    ...
    for (const target of targets) {
        ...
        if (border)
            target.style.setProperty('--divider-color', border);
        if (shadow)
            target.style.setProperty('--dialog-box-shadow', shadow);
        if (radius)
...
function paintOpenMoreInfoDialogs(tokens) {
    ...
    const walk = (root, depth = 0) => {
        if (!root || depth > 10)
            return;
        const list = root.querySelectorAll?.('ha-more-info-dialog, ha-dialog, ha-adaptive-dialog, wa-dialog, more-info-weather, more-info-content');
    ...
    for (const el of roots) {
        ...
        if (tokens.panel && /DIALOG|MORE-INFO-WEATHER|MORE-INFO-CONTENT/i.test(el.tagName)) {
            el.style.background = tokens.panel;
        }
    }
}
...
`;
function firstToken(computed, keys) {
    for (const key of keys) {
        const value = computed.getPropertyValue(key).trim();
        if (value)
            return value;
...
function openWeatherDialog(host, hass, entityId, forecast) {
    ...
    if (humidity)
        metrics.push({ label: '湿度', value: humidity });
    if (pressure)
        metrics.push({ label: '气压', value: pressure });
    if (wind)
        metrics.push({ label: '风速', value: windBearing ? `${wind} ${windBearing}` : wind });
    if (visibility)
    ...
    <div class="wx-scrim" @click=${onClose}>
      <div class="wx-card" @click=${(e) => e.stopPropagation()}>
        <div class="wx-head">
          <div>
        ...
        <div class="wx-now">
          ...
          <div>
            <div>
              ...
              ${todayHigh && todayLow ? b `<span class="wx-hl">${todayHigh}/${todayLow}</span>` : A}
            </div>
        ...
        return b `
                <div class="wx-day">
                  ...
                  ${low ? b `<div class="wx-day-low">${low}</div>` : A}
                </div>
              `;
    })}
          </div>
      ...
      <div class="welcome-group">
        ...
        <div class="weather-with-meta" style="display:flex !important;align-items:flex-start !important;margin-top:var(--sp-space-md,12px) !important;grid-area:unset !important;width:fit-content !important;max-width:540px !important;">
          ...
          <div
            ...
            @pointerdown=${(event) => startHomeMetaDrag(event, ctx)}
          >
...
function hasCompletePosition(pos) {
    return normalizePercent(pos?.x) !== undefined && normalizePercent(pos?.y) !== undefined;
}
function startHomeMetaDrag(event, ctx) {
    if (event.button !== undefined && event.button !== 0)
        return;
    if (event.pointerType === 'mouse')
        event.preventDefault();
    const card = event.currentTarget;
    const stage = card.closest('.stage-grid');
    if (!stage)
        return;
...
function readStoredHomeMetaPosition() {
    try {
...
function writeStoredHomeMetaPosition(x, y) {
    try {
