...
export function renderDeviceCard(
  ...
  const action = isClimate ? 'climate-control' : (isMedia ? 'play-pause' : (CONTROLLABLE_DOMAINS.has(device.detail) && !isCover && !isValve ? 'toggle' : 'more-info'));
